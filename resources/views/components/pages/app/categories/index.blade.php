            ...
            <table class="w-full text-sm text-left text-slate-500 dark:text-slate-400">
                ...
                @foreach($categories as $category)
                  <tr class="border-b border-b-slate-300 dark:border-b-slate-600 hover:bg-orange-100 dark:hover:bg-orange-800">
                    ...
                    <td class="px-4 py-3 whitespace-nowrap">{{ $category->created_at }}</td>
                    <td class="px-4 py-3 whitespace-nowrap">{{ $category->updated_at }}</td>
                    <td class="px-4 py-3 flex space-x-4">
                      @foreach($actions as $action)
                        @if($action["name"] === "Delete")
                          <form action="{{ route($action["uri"])."/".$category->id }}" method="post">
                            @method("delete")
                            @csrf
                        ...
                        @else
                          <button onclick="window.location='{{ route($action["uri"])."/".$category->id."/edit" }}'" class="text-{{ $action["color"] }}-400 dark:text-{{ $action["color"] }}-500 hover:text-{{ $action["color"] }}-600 dark:hover:text-{{ $action["color"] }}-300 rounded-md">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" aria-hidden="true">
                              {!! $action["icon"] !!}
                            </svg>
                          </button>
