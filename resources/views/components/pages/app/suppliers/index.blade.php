    ...
    <div class="grow h-0 shadow-sm sm:rounded-lg bg-slate-50 dark:bg-slate-900 mx-4 mb-4 grid grid-rows-6">
        <div class="flex flex-col flex-none px-4 py-3 space-y-3 lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:space-x-4">
            <div class="flex items-center flex-1 space-x-4">
                <h5>
                    <span class="text-slate-500">All Suppliers:</span>
                    <span class="dark:text-white">{{ $suppliers->count() }}</span>
                </h5>
            </div>
                ...
                @foreach($suppliers as $supplier)
                    <tr class="border-b border-b-slate-300 dark:border-b-slate-600 hover:bg-orange-100 dark:hover:bg-orange-800">
                      <td class="px-4 py-3 whitespace-nowrap">{{ $loop->iteration }}</td>
                        <th scope="row" class="px-4 py-3 font-medium text-slate-900 whitespace-nowrap dark:text-white">{{ $supplier->name }}</th>
                        <td class="px-4 py-3 whitespace-nowrap">{{ $supplier->full_address }}</td>
                        <td class="px-4 py-3 whitespace-nowrap">{{ $supplier->phone_number }}</td>
                        <td class="px-4 py-3 whitespace-nowrap">{{ $supplier->created_at }}</td>
                        <td class="px-4 py-3 whitespace-nowrap">{{ $supplier->updated_at }}</td>
                      <td class="px-4 py-3 flex space-x-4">
                        @foreach($actions as $action)
                          @if($action["name"] === "Delete")
                            <form action="{{ route($action["uri"])."/".$supplier->id }}" method="post">
                              @method("delete")
                              @csrf
                          ...
                          @else
                            <button onclick="window.location='{{ route($action["uri"])."/".$supplier->id."/edit" }}'" class="text-{{ $action["color"] }}-400 dark:text-{{ $action["color"] }}-500 hover:text-{{ $action["color"] }}-600 dark:hover:text-{{ $action["color"] }}-300 rounded-md">
                              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" aria-hidden="true">
                                {!! $action["icon"] !!}
                              </svg>
                            </button>
