            ...
            <table class="w-full text-sm text-left text-slate-500 dark:text-slate-400">
                ...
                @foreach($units as $unit)
                    <tr class="border-b border-b-slate-300 dark:border-b-slate-600 hover:bg-orange-100 dark:hover:bg-orange-800">
                        <th scope="row" class="px-4 py-3 font-medium text-slate-900 whitespace-nowrap dark:text-white">{{ $unit->name }}</th>
                        <td class="px-4 py-3">{{ $unit->created_at }}</td>
                        <td class="px-4 py-3">{{ $unit->updated_at->diffForHumans() }}</td>
